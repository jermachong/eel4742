...
//// if button is pressed & other button already running, turn off both LEDS until both buttons are released
int main(void)
{
    WDTCTL = WDTPW | WDTHOLD; // Stop the Watchdog timer
    PM5CTL0 &= ~LOCKLPM5;     // Disable GPIO power-on default high-impedance
    ...
    for (;;)
    {
        ...
        b2 = !(P1IN & BUT2); // green button pushed
        ...
        if (b1 && b2) // lockout
        { // red and green buttons are pushed, turn off both LEDs
            do
            {
                P1OUT &= ~redLED;
                P9OUT &= ~greenLED;
