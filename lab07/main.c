...
int main() {
  ...
  for (;;) {
    ...
    // 2. Did the Button ISR say the button was pressed?
    if (button_pressed == 1) {
      update_time(time);

      // Optional: Reset the timer so you don't instantly log
      // right after changing the time.
      timer_seconds = 0;
  ...
  void update_time(char input_time[]) {
    ...
    while (1) {
      ...
      if (t != 0) { // ONLY act if a real character was received
        ...
        if (t >= '0' && t <= '9') {
          temp_buf[i] = t;
          // uart_write_char(t); // Echo so the user can see what they are
          // typing! uart_write_char('\n');
          i++;
        }
...
#pragma vector = TIMER0_A0_VECTOR
  __interrupt void Timer_A_ISR(void) {
    ...
    if (timer_seconds >= seconds_scalar) {
      log_ready = 1;     // Ring the bell for the main loop!
      timer_seconds = 0; // Reset the counter
    }

    // Note: Depending on your MSP430 family, CCIFG might auto-clear.
    // If not, clear it here: TA0CCTL0 &= ~CCIFG;
  }